...
namespace api {
    ...
    */
    ...
    bool NetworkUtils::sendMessageToAddress(const string& ip, int port, const message& msg, int timeout_ms, int max_retries) {
        for (int attempt = 0; attempt < max_retries; attempt++) {
            // create connector socket (outgoing connection)
            int sockfd = createConnectorSocket(timeout_ms);
            if (sockfd < 0) continue;

            // attempt to connect to address of recipient
            if (connectToAddress(sockfd, ip, port)) {
                bool success = sendMessageAndWaitForAck(sockfd, msg);
                close(sockfd);
    ...
    bool NetworkUtils::sendMessageAndWaitForAck(int connector_socket, const message& msg) {
        if (!sendMessage(connector_socket, msg)) {
            return false;
        }

        message ack_response;
        if (!recvMessage(connector_socket, ack_response)) {
            return false;
        }
    ...
    bool NetworkUtils::requestFromZipper(const string& ip, int port, const message& msg, message& resp, int timeout_ms, int max_retries) {
        for (int attempt = 0; attempt < max_retries; attempt++) {
            // create connector socket (outgoing connection)
            int sockfd = createConnectorSocket(timeout_ms);
            if (sockfd < 0) continue;
    ...
    */
    ...
    int NetworkUtils::createConnectorSocket(int timeout_ms) {
        // bind to socket
    ...
    bool NetworkUtils::connectToAddress(int connector_socket, const string& ip, int port) {
    ...
    int NetworkUtils::createListeningSocket(const string& ip, int port, bool reuse_addr) {
